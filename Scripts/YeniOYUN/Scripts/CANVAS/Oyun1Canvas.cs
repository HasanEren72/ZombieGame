...
public class Oyun1Canvas : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        karekterKontrol2 =Object.FindObjectOfType<KarekterKontrol2>();
    }
    ...
    { 
        ...
        for (int i = 0; i < 10; i++)  //1.Dalga rasgele 10 zombi
        {
            ...
            GameObject nesne = Instantiate(ClonZombiler1[zombİndis], new Vector3(xpozisyon, 0, zpozisyon), randomRotation);
            nesnesayisi++;
        }
    ...
    {                //fonk çağırıp stinge döbüştürerek atadık
        ...
        if (karekterKontrol2.hayattaMi ==false)
        {
            StartCoroutine(skorlar_ekleme());//skorları veritabanına ekler
            karekterKontrol2.hayattaMi = true;
        }
    ...
    public void Dalga2()    //2.Dalga farklı özelliklere sahip rasgele 20 zombi
    {
        ...
        for (int i = 0; i < 20; i++)  //1.Dalga rasgele 10 zombi
        {
            ...
            GameObject nesne = Instantiate(ClonZombiler2[zombİndis], new Vector3(xpozisyon, 0, zpozisyon), randomRotation);
            nesnesayisi++;
        }
    }
    public void Dalga3()    //3.Dalga farklı özelliklere sahip rasgele 30 zombi
    {
        ...
        for (int i = 0; i < 40; i++)  //1.Dalga rasgele 10 zombi
        {
            ...
            GameObject nesne = Instantiate(ClonZombiler3[zombİndis], new Vector3(xpozisyon, 0, zpozisyon), randomRotation);
            nesnesayisi++;
        }
    }
    ...
    public void OyunuDevamEt()
    {
        ...
        for (int i = 0; i < MutantNesneleri.Length; i++) //oyun durmuşsa " tüm Mutant SESLERİNİ KESMEK İÇİN" 
        {
            AudioSource mutantSesi = MutantNesneleri[i].GetComponent<AudioSource>();
            if (mutantSesi != null)  // Mutant nesnesinde ses bileşeni varsa
            {
                mutantSesi.enabled = true;  // mutant sesini kapatır
            }
        }

        Physics.queriesHitTriggers = true; // Raycast işlemini aktif hale getirir.
    ...
    public void OyunuDurdur()
    {
        ...
        for (int i = 0; i < MutantNesneleri.Length; i++) //oyun durmuşsa " tüm Mutant SESLERİNİ KESMEK İÇİN" 
        {
            ...
            if (mutantSesi != null)  // Mutant nesnesinde ses bileşeni varsa
            {
                mutantSesi.enabled = false;  // mutant sesini kapatır
            }
        }

        Physics.queriesHitTriggers = false; // Raycast işlemini devre dışı bırakır.
    }
